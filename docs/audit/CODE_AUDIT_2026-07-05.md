...
- **Resume is metadata-driven:** every posted comment carries a single-line,
  versioned zlib/base64 `AGENT_LOOP_META` marker; oversized metadata uses
  preceding `AGENT_LOOP_SIDECAR` transport comments. Rounds reconstruct
  deterministically from GitHub comments rather than local state alone, so a
  crashed process on a different machine can resume.
...
| R-2 | Low | **`state_manager._fetch_issue_comments` splits `gh --jq '.[].body'` output on newlines**, so multi-line comment bodies arrive as one fragment per line. This is safe today only because resume consumes single-line `AGENT_LOOP_META` markers — a fragile invariant that deserves a comment, or a switch to JSON output (`--json body`) to get real comment boundaries. |
