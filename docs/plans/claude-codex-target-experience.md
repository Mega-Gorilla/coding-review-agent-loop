...
| `BLOCKED` | 自動継続できないfinding、no-progress、外部判断待ち |
...
| Claude requests clarification | 自動変更を止める | 質問、影響、選択肢 | 回答を与えて再開 |
...
- Clarification回答を同じterminalのstdinで受け取るか
- controllerを一度終了し、resume commandへ回答を渡す方式にするか
- GitHub commentによる非同期回答を将来サポートするか
...
- GitHub comment経由の非同期clarification
...
| Q-005 | clarification回答をどう渡すか | 無人継続とuser interactionへ影響 | terminalで停止しresume時に回答 |
