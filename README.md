...
| Remote monitoring | Progress is only visible by tailing logs in a terminal; checking in requires SSH access to the machine running the loop. | The host session is a live Claude Code session, so you can monitor progress, read status updates, and answer clarification questions from any device that can reach the session — including a phone via the Claude web or mobile app. |
