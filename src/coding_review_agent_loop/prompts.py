...
def format_issue_context(issue_context: IssueContext, *, max_chars: int = 24_000) -> str:
    ...
    visible_comments = tuple(comment for comment in issue_context.comments if not _is_salvage_breadcrumb_comment(comment) and not (comment.body and is_round_transport_sidecar(comment.body)))
    if visible_comments:
        comments = [
            "\n".join(
                [
